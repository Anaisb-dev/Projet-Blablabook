<script>
	...
	const styles = {
		...
		myLibraryButton: "mt-3 w-[200px] h-[40px] rounded-xl border flex items-center justify-center bg-white hover:bg-[#F2E0D0] transition-colors duration-200"
	};
</script>

<button class={`${styles[variant]} ${active ? "bg-[#F2E0D0]" : ""}`} on:click>
	<slot></slot>
</button>
	<!-- styles.primary est un fallback au cas où une variante n'est pas définie, par défaut le bouton sera stylé avec la classe primary -->
