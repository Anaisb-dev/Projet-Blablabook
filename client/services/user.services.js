...
export const getUser = async () => {
    return await api("/api/users/settings", "GET");
};


// Modifier les informations de l'utilisateur connecté
export const updateUser = async (data) => {
    const response = await api("/api/users/settings", "PATCH", data);
...
export const updatePassword = async (password, confirmPassword) => {
    return await api("/api/users/password", "PATCH", {
        password,
        confirmPassword
    });
};
...
export const deleteUser = async () => {
    return await api("/api/users/profile", "DELETE");
};
